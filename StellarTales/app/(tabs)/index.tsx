...
function Header({ date, locationDenied }: { date: Date; locationDenied: boolean }) {
  ...
  return (
    <View style={styles.header}>
      ...
      {locationDenied && (
        <Text style={styles.locationNote}>Using default location · Allow location for your sky</Text>
      )}
    </View>
  );
}
  ...
  return (
    <View style={styles.center}>
      ...
      <Text style={styles.emptyBody}>
        Try again after sunset, or browse all objects in the Explore tab.
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  locationNote: { color: Space.textMuted, fontSize: 11, marginTop: 4 },
  loadingStars: { fontSize: 40, color: Space.accent },
  loadingLabel: { color: Space.textSecondary, fontSize: 14, marginTop: 12 },
  emptyIcon: { fontSize: 48 },
  emptyTitle: { color: Space.text, fontSize: 18, fontWeight: '600', marginTop: 16, textAlign: 'center' },
  emptyBody: { color: Space.textSecondary, fontSize: 14, lineHeight: 20, marginTop: 10, textAlign: 'center' },
  separator: { height: 16 },
});
